...
namespace EstuarineCirculationModeling
{
    public class AsymmTidalMix
    {
        ...
        private readonly int numCells = 70; // Total: 30 main + 20 branch 1 + 20 branch 2
        ...
        private readonly RichardsonNumDepAndSSIMix mixingModel; // Mixing model
        private readonly SimpsonHunterMechanismParam tidalEffectsModel; // Tidal effects model
        private readonly BifurcatedEstuaryModels bifurcatedModel; // Bifurcated estuary model
        ...
        {
            InitializeWindow();
            mixingModel = new RichardsonNumDepAndSSIMix(numSigmaLayers); // Initialize mixing model
            tidalEffectsModel = new SimpsonHunterMechanismParam(numSigmaLayers); // Initialize tidal effects model
            bifurcatedModel = new BifurcatedEstuaryModels(); // Initialize bifurcated estuary model
            InitializeGrid();
            currentTime = 0.0;
            tidalPhase = 0.0;
        ...
        {
            cells = bifurcatedModel.InitializeBifurcatedGrid();
        }
        ...
        private void UpdateSimulation()
        {
            ...
            foreach (Cell cell in cells)
            {
                ...
                for (int k = 0; k < numSigmaLayers; k++)
                {
                    ...
                    else if (i == 49 || i == 69) // Branch ends (numCellsMain - 1 + numCellsBranch, numCellsMain - 1 + 2 * numCellsBranch)
                    {
                        double adjustedTidalVelocity = bifurcatedModel.AdjustTidalVelocity(tidalVelocity, i, cells);
                        uStar[i, k] = 0.1 * (1.0 - sigma) + adjustedTidalVelocity * (1.0 - sigma); // River inflow + scaled tidal influence
                        vStar[i, k] = 0.0;
                        wStar[i, k] = 0.0;
                    ...
                    // Turbulent viscosity (use mixing model)
                    double richardsonNumber;
                    double nuT = mixingModel.ComputeAdjustedTurbulentViscosity(cell, k, depth, sigmaStep, out richardsonNumber);

                    // Stokes drift contribution
                    double stokesDrift = tidalEffectsModel.ComputeStokesDrift(depth, sigma, tidalPhase);

                    // Internal tide effect
                    double internalTideW;
                    double internalTideTKE = 0.0;
                    internalTideW = tidalEffectsModel.ComputeInternalTideEffect(cell, k, depth, sigmaStep, tidalPhase, out internalTideTKE);
                    ...
                    if (i > 0 && cell.U[k] > 0)
                    {
                        ...
                        {
                            if (neighborIdx < i) // Upstream neighbor
                            {
                                uAdv += (cell.U[k] + stokesDrift) * (cell.U[k] - cells[neighborIdx].U[k]) / avgCellWidth;
                                vAdv += cell.U[k] * (cell.V[k] - cells[neighborIdx].V[k]) / avgCellWidth;
                            }
                        }
                    }
                    else if (i < numCells - 1 && cell.U[k] < 0)
                    {
                        foreach (int neighborIdx in cell.Neighbors)
                        {
                            if (neighborIdx > i) // Downstream neighbor
                            {
                                uAdv += (cell.U[k] + stokesDrift) * (cells[neighborIdx].U[k] - cell.U[k]) / avgCellWidth;
                                vAdv += cell.U[k] * (cells[neighborIdx].V[k] - cell.V[k]) / avgCellWidth;
                            }
                        }
                    }
                    if (k < numSigmaLayers - 1 && cell.W[k] > 0)
                    {
                        wAdv = (cell.W[k] + internalTideW) * (cell.U[k] - cell.U[k + 1]) / (sigmaStep * depth);
                    }
                    else if (k > 0 && cell.W[k] < 0)
                    {
                        wAdv = (cell.W[k] + internalTideW) * (cell.U[k - 1] - cell.U[k]) / (sigmaStep * depth);
                    }
                    ...
                    double coriolisV = -coriolisParameter * (cell.U[k] + stokesDrift);
                    double density = referenceDensity + 0.8 * cell.Salinity[k];
                    double buoyancy = -gravity * (density - referenceDensity) / referenceDensity;

                    // Bed friction
                    double bedFrictionU = (k == 0) ? -bedShearStress * asymmetryFactor * (cell.U[k] + stokesDrift) / referenceDensity : 0.0;
                    ...
                    wStar[i, k] = cell.W[k] + dt * (-wAdv + wDiff + buoyancy + internalTideW);
            ...
            for (int iter = 0; iter < 20; iter++) // SOR iterations
            {
                for (int i = 0; i < numCells; i++)
                {
                    ...
                    for (int k = 0; k < numSigmaLayers; k++)
                    {
                        if (i == 0 || i == 49 || i == 69) continue; // Skip boundaries
                        double div = 0.0;
                        foreach (int neighborIdx in cell.Neighbors)
                        {
                            if (neighborIdx < i)
                                div += (uStar[i, k] - uStar[neighborIdx, k]) / avgCellWidth;
                            else if (neighborIdx > i)
                                div += (uStar[neighborIdx, k] - uStar[i, k]) / avgCellWidth;
                        }
                        if (k < numSigmaLayers - 1)
            ...
            foreach (Cell cell in cells)
            {
                ...
                for (int k = 0; k < numSigmaLayers; k++)
                {
                    ...
                    {
                        if (neighborIdx > i)
                            dpdx += (pressure[neighborIdx, k] - pressure[i, k]) / avgCellWidth;
                        else if (neighborIdx < i)
                            dpdx += (pressure[i, k] - pressure[neighborIdx, k]) / avgCellWidth;
                    }
            ...
            // Step 4: Update turbulence (k-ε model with Ri-dependent and shear-strain mixing)
            foreach (Cell cell in cells)
            {
                int i = cells.IndexOf(cell);
                double depth = cell.Depth;
                ...
                for (int k = 0; k < numSigmaLayers; k++)
                {
                    ...
                    // Internal tide TKE production
                    double internalTideTKE;
                    tidalEffectsModel.ComputeInternalTideEffect(cell, k, depth, sigmaStep, tidalPhase, out internalTideTKE);

                    // Bifurcated mixing dynamics
                    double kProd = 0.0;
                    bifurcatedModel.AdjustMixingDynamics(cell, i, k, depth, sigmaStep, tidalVelocity, ref nuT, ref kProd, cells);

                    // Turbulent kinetic energy (k) equation
                    double kAdv = 0.0, kDiff = 0.0;
                    if (i > 0 && cell.U[k] > 0)
                    {
                        foreach (int neighborIdx in cell.Neighbors)
                        {
                            if (neighborIdx < i)
                                kAdv += cell.U[k] * (cell.K[k] - cells[neighborIdx].K[k]) / avgCellWidth;
                        }
                    }
                    else if (i < numCells - 1 && cell.U[k] < 0)
                    {
                        ...
                        {
                            if (neighborIdx > i)
                                kAdv += cell.U[k] * (cells[neighborIdx].K[k] - cell.K[k]) / avgCellWidth;
                        }
                    }
                    ...
                    kProd += internalTideTKE; // Add internal tide TKE production
                    ...
                    if (i > 0 && cell.U[k] > 0)
                    {
                        ...
                        {
                            if (neighborIdx < i)
                                eAdv += cell.U[k] * (cell.Epsilon[k] - cells[neighborIdx].Epsilon[k]) / avgCellWidth;
                        }
                    }
                    else if (i < numCells - 1 && cell.U[k] < 0)
                    {
                        ...
                        {
                            if (neighborIdx > i)
                                eAdv += cell.U[k] * (cells[neighborIdx].Epsilon[k] - cell.Epsilon[k]) / avgCellWidth;
                        }
                    }
                    ...
                    // Salinity advection and diffusion with tidal straining
                    ...
                    if (i > 0 && cell.U[k] > 0)
                    {
                        ...
                        {
                            if (neighborIdx < i)
                                salinityChange -= cell.U[k] * (cell.Salinity[k] - cells[neighborIdx].Salinity[k]) / avgCellWidth;
                        }
                    }
                    else if (i < numCells - 1 && cell.U[k] < 0)
                    {
                        ...
                        {
                            if (neighborIdx > i)
                                salinityChange -= cell.U[k] * (cells[neighborIdx].Salinity[k] - cell.Salinity[k]) / avgCellWidth;
                        }
                    }
                    if (k < numSigmaLayers - 1)
                    {
                    ...
                    double straining = tidalEffectsModel.ComputeTidalStraining(cell, k, depth, sigmaStep, tidalVelocity, cells, i, avgCellWidth);
                    salinityChange += straining * dt;
                    ...
                    if (i > 0 && cell.U[k] > 0)
                    {
                        ...
                        {
                            if (neighborIdx < i)
                                turbidityChange -= cell.U[k] * (cell.Turbidity[k] - cells[neighborIdx].Turbidity[k]) / avgCellWidth;
                        }
                    }
                    else if (i < numCells - 1 && cell.U[k] < 0)
                    {
                        ...
                        {
                            if (neighborIdx > i)
                                turbidityChange -= cell.U[k] * (cells[neighborIdx].Turbidity[k] - cell.Turbidity[k]) / avgCellWidth;
                        }
                    }
                    if (k < numSigmaLayers - 1)
                    {
            ...
            // Store velocity profile for hodograph (sample from main channel and both branches)
            if ((currentTime % (tidalPeriod / 12)) < dt)
            {
                int numCellsMain = 30; // Matches BifurcatedEstuaryModels
                int numCellsBranch = 20; // Matches BifurcatedEstuaryModels
                int[] sampleIndices = { numCellsMain / 2, numCellsMain + numCellsBranch / 2, numCellsMain + numCellsBranch + numCellsBranch / 2 };
                foreach (int idx in sampleIndices)
                {
                    if (idx < cells.Count)
                    {
                        Cell sampleCell = cells[idx];
                        double[] uProfile = new double[numSigmaLayers];
                        double[] wProfile = new double[numSigmaLayers];
                        Array.Copy(sampleCell.U, uProfile, numSigmaLayers);
                        Array.Copy(sampleCell.W, wProfile, numSigmaLayers);
        ...
        private void UpdateOutputConsole()
        {
            ...
            double avgRichardsonNumber = mixingModel.ComputeAverageRichardsonNumber(cells, estuaryLength / numCells);
            ...
            outputConsoleTextBox.AppendText($"Time: {currentTime:F2}s | Avg TKE: {avgK:F6} m²/s² | Max Velocity: {maxVelocity:F4} m/s | Avg Salinity: {avgSalinity:F2} PSU | Avg Turbidity: {avgTurbidity:F4} | Max Shear: {maxShear:F4} 1/s | Avg Ri: {avgRichardsonNumber:F4} | Tide: {(tidalPhase >= 0 && tidalPhase < Math.PI ? "Flood" : "Ebb")}\r\n");
        }
        ...
        private void VisualizationPanel_Paint(object sender, PaintEventArgs e)
        {
            ...
            g.DrawString($"Tidal Phase: {(tidalPhase >= 0 && tidalPhase < Math.PI ? "Flood" : "Ebb")}",
                         new Font("Verdana", 10),
                         new SolidBrush(tidalPhase >= 0 && tidalPhase < Math.PI ? Color.Red : Color.Blue),
                         10, 10);
            ...
            maxTurbulence = Math.Min(maxTurbulence, 1e-3);
            maxVelocity = Math.Max(maxVelocity, 0.01);
            maxVelocity = Math.Min(maxVelocity, 2.0);
            maxSalinity = Math.Max(maxSalinity, 0.01);
            maxSalinity = Math.Min(maxSalinity, 35.0);
            maxTurbidity = Math.Max(maxTurbidity, 0.01);
            maxTurbidity = Math.Min(maxTurbidity, 100.0);
            ...
            if (showSalinityCheckBox.Checked)
            {
                ...
                for (int i = 0; i < numCells - 1; i++)
                {
                    ...
                    for (int k = 0; k < numSigmaLayers - 1; k++)
                    {
                        ...
                        {
                            using (Pen pen = new Pen(Color.FromArgb(tidalPhase >= 0 && tidalPhase < Math.PI ? 255 : 128, 0, 255, 0),
                                                     tidalPhase >= 0 && tidalPhase < Math.PI ? 2f : 1f))
                            {
            ...
            if (showTurbidityCheckBox.Checked)
            {
                ...
                for (int i = 0; i < numCells - 1; i++)
                {
                    ...
                    if ((avgTurb1 - threshold) * (avgTurb2 - threshold) <= 0)
                    {
                        ...
                        y = Math.Max(0, Math.Min(height, y));
                        using (Pen pen = new Pen(Color.FromArgb(tidalPhase >= 0 && tidalPhase < Math.PI ? 255 : 128, 139, 69, 19),
                                                 tidalPhase >= 0 && tidalPhase < Math.PI ? 2f : 1f))
                        {
                            g.DrawLine(pen, x1, y, x2, y);
                        }
            ...
            for (int i = 0; i < numCells - 1; i++)
            {
                ...
                using (Pen turbPen = new Pen(Color.FromArgb(tidalPhase >= 0 && tidalPhase < Math.PI ? 255 : 128, 0, 0, 255),
                                             tidalPhase >= 0 && tidalPhase < Math.PI ? 2f : 1f))
                {
                    g.DrawLine(turbPen, x1, y1Turb, x2, y2Turb);
                }
                using (Pen velPen = new Pen(Color.FromArgb(tidalPhase >= 0 && tidalPhase < Math.PI ? 255 : 128, 255, 0, 0),
                                            tidalPhase >= 0 && tidalPhase < Math.PI ? 2f : 1f))
                {
            ...
            if (showVectorsCheckBox.Checked)
            {
                for (int i = 0; i < numCells; i += 5)
                {
                    ...
                    for (int k = 0; k < numSigmaLayers; k += 2)
                    {
                        ...
                        x = Math.Max(0, Math.Min(width, x));
                        y = Math.Max(0, Math.Min(height, y));
                        using (Pen pen = new Pen(Color.FromArgb(tidalPhase >= 0 && tidalPhase < Math.PI ? 255 : 128, 0, 0, 0),
                                                 tidalPhase >= 0 && tidalPhase < Math.PI ? 2f : 1f))
                        {
            ...
            if (showStreamlinesCheckBox.Checked)
            {
                for (int start = 0; start < numCells; start += 10)
                {
                    ...
                    {
                        using (Pen pen = new Pen(Color.FromArgb(tidalPhase >= 0 && tidalPhase < Math.PI ? 255 : 128, 128, 0, 128),
                                                 tidalPhase >= 0 && tidalPhase < Math.PI ? 2f : 1f))
                        {
        ...
        private void HodographPanel_Paint(object sender, PaintEventArgs e)
        {
            ...
            int[] sampleIndices = { numCellsMain / 2, numCellsMain + numCellsBranch / 2, numCellsMain + numCellsBranch + numCellsBranch / 2 };
            Color[] sampleColors = { Color.Blue, Color.Green, Color.Red };
            ...
            for (int k = 0; k < numSigmaLayers; k += 2)
            {
                for (int s = 0; s < sampleIndices.Length; s++)
                {
                    ...
                    {
                        using (Pen pen = new Pen(Color.FromArgb(tidalPhase >= 0 && tidalPhase < Math.PI ? 255 : 128, sampleColors[s]),
                                                 tidalPhase >= 0 && tidalPhase < Math.PI ? 2f : 1f))
                        {
                            g.DrawLines(pen, hodographPoints.ToArray());
                        }
                        PointF currentPoint = hodographPoints[hodographPoints.Count - 1];
                        using (Brush brush = new SolidBrush(sampleColors[s]))
                        {
            ...
            {
                g.DrawString("Main Channel", font, Brushes.Blue, 10, 10);
                g.DrawString("Branch 1", font, Brushes.Green, 10, 30);
                g.DrawString("Branch 2", font, Brushes.Red, 10, 50);
            }
        }
