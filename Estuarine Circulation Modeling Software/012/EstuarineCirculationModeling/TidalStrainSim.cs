...
namespace EstuarineCirculationModeling
{
    public class TidalStrainSim
    {
        ...
        {
            // Initialize salinity and temperature
        ...
        private void InitializeForm()
        {
            ...
            visualizationModeComboBox.Items.AddRange(new[] { "Salinity", "Vorticity", "Density" });
        ...
        {
            return rho0 * (1.0 - alpha * (temperatureProfile[i, j] - T0) + beta * (salinityProfile[i, j] - S0));
        }
        ...
        private void UpdateSimulation()
        {
            ...
            for (int i = 1; i < gridX - 1; i++)
            {
                for (int j = 1; j < gridZ - 1; j++)
                {
                    ...
                    // Diffusion term (using kinematicViscosity for salinity diffusivity)
            ...
            for (int i = 0; i < gridX; i++)
            {
                for (int j = 1; j < gridZ - 1; j++)
                {
                    ...
                    double drhodz = -rho0 * (alpha * dTdz - beta * dsdz); // Density gradient
                    double N2 = -9.81 / rho0 * drhodz; // Buoyancy frequency squared
        ...
        private void TimeSeriesBox_Paint(object sender, PaintEventArgs e)
        {
            ...
            g.DrawString("Ri / Rρ", new Font("Consolas", 8), Brushes.Black, 30, 10);

            // Find max values for scaling
            double maxRi = 0.0, maxDensityRatio = 0.0;
            ...
            for (int i = 1; i < historyIndex; i++)
            {
                ...
                int y1 = (int)(height - 30 - (richardsonHistory[i - 1] / maxRi) * (height - 40) / 2);
                int y2 = (int)(height - 30 - (richardsonHistory[i] / maxRi) * (height - 40) / 2);
            ...
            for (int i = 1; i < historyIndex; i++)
            {
                ...
                int y1 = (int)(height - 30 - (densityRatioHistory[i - 1] / maxDensityRatio) * (height - 40) / 2);
                int y2 = (int)(height - 30 - (densityRatioHistory[i] / maxDensityRatio) * (height - 40) / 2);
