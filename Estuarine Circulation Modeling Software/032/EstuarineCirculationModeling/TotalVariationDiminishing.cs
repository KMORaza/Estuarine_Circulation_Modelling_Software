...
namespace EstuarineCirculationModeling
{
    public class TotalVariationDiminishing
    {
        ...
        private CheckBox useUnstructuredGridCheckBox;
        ...
        private bool useUnstructuredGrid = false;
        ...
        private double[] nodeSalinity;
        private double[] nodeTemperature;
        private double[] nodeVelocityX;
        private double[] nodeVelocityZ;
        private double[] nodeDensity;
        private double[] nodeK;
        private double[] nodeEpsilon;
        private double[] nodeOmega;
        private double[] nodeEddyViscosity;
        private double[] nodeEddyDiffusivity;

        private class Node
        {
            public double X { get; set; }
            public double Z { get; set; }
            public List<int> NeighborTriangles { get; set; } = new List<int>();
        }

        private class Triangle
        {
            public int[] Nodes { get; set; } = new int[3];
        ...
        private void InitializeFields()
        {
            ...
            {
                InitializeUnstructuredGrid();
                InitializeUnstructuredFields();
            }
            else
            {
                ...
                for (int i = 0; i < gridSizeX; i++)
                {
                    ...
                    for (int j = 0; j < gridSizeZ; j++)
                    {
                        ...
                        densityField[i, j] = Math.Max(950.0, Math.Min(1050.0, densityField[i, j]));
        ...
        private void InitializeUnstructuredGrid()
        {
            nodes = new List<Node>();
            triangles = new List<Triangle>();
            int nx = 40, nz = 25; // Coarser grid for unstructured mesh
            double dx_unstruct = estuaryLength / (nx - 1);
            double maxDepth = estuaryDepth;

            // Generate nodes with bathymetry
            for (int i = 0; i < nx; i++)
            {
                double x = i * dx_unstruct;
                // Bathymetry: shallower near river (x < 200)
                double localDepth = maxDepth * (0.5 + 0.5 * Math.Min(1.0, x / 200.0));
                double dz_unstruct = localDepth / (nz - 1);
                for (int j = 0; j < nz; j++)
                {
                    double z = j * dz_unstruct;
                    nodes.Add(new Node { X = x, Z = z });
                }
            }

            // Generate triangles (simple structured triangulation)
            for (int i = 0; i < nx - 1; i++)
            {
                for (int j = 0; j < nz - 1; j++)
                {
                    ...
                    int n2 = (i + 1) * nz + j;
                    int n3 = n2 + 1;
                    Triangle t1 = new Triangle { Nodes = new[] { n0, n2, n3 } };
                    Triangle t2 = new Triangle { Nodes = new[] { n0, n3, n1 } };
                    t1.Area = ComputeTriangleArea(nodes[n0], nodes[n2], nodes[n3]);
                    t2.Area = ComputeTriangleArea(nodes[n0], nodes[n3], nodes[n1]);
                    triangles.Add(t1);
                    triangles.Add(t2);
                    nodes[n0].NeighborTriangles.Add(triangles.Count - 2);
                    nodes[n0].NeighborTriangles.Add(triangles.Count - 1);
                    nodes[n1].NeighborTriangles.Add(triangles.Count - 1);
                    nodes[n2].NeighborTriangles.Add(triangles.Count - 2);
                    nodes[n3].NeighborTriangles.Add(triangles.Count - 2);
                    nodes[n3].NeighborTriangles.Add(triangles.Count - 1);
                }
            }

            // Initialize field arrays
            nodeSalinity = new double[nodes.Count];
            nodeTemperature = new double[nodes.Count];
            nodeVelocityX = new double[nodes.Count];
            nodeVelocityZ = new double[nodes.Count];
            nodeDensity = new double[nodes.Count];
            nodeK = new double[nodes.Count];
            nodeEpsilon = new double[nodes.Count];
            nodeOmega = new double[nodes.Count];
            nodeEddyViscosity = new double[nodes.Count];
            nodeEddyDiffusivity = new double[nodes.Count];
        }

        private double ComputeTriangleArea(Node n0, Node n1, Node n2)
        {
            double x0 = n0.X, z0 = n0.Z;
            double x1 = n1.X, z1 = n1.Z;
            double x2 = n2.X, z2 = n2.Z;
            return 0.5 * Math.Abs(x0 * (z1 - z2) + x1 * (z2 - z0) + x2 * (z0 - z1));
        }

        private void InitializeUnstructuredFields()
        {
            double x0 = 500.0;
            for (int n = 0; n < nodes.Count; n++)
            {
                double x = nodes[n].X;
                double z = nodes[n].Z;
                double localDepth = estuaryDepth * (0.5 + 0.5 * Math.Min(1.0, x / 200.0));
                nodeSalinity[n] = 17.5 * (1.0 + Math.Tanh((x - x0) / 50.0)) * (1.0 - 0.5 * z / localDepth);
                nodeTemperature[n] = Math.Max(15.0, 20.0 + 2.0 * (1.0 - Math.Tanh((x - x0) / 50.0)) - 2.0 * z / localDepth);
                nodeDensity[n] = rho0 * (1.0 - alpha * (nodeTemperature[n] - T0) + beta_s * (nodeSalinity[n] - S0));
                nodeDensity[n] = Math.Max(950.0, Math.Min(1050.0, nodeDensity[n]));
                nodeVelocityX[n] = tidalAmplitude * (1.0 - z / localDepth);
                nodeVelocityZ[n] = -0.02 * (z / localDepth);
                nodeK[n] = 1e-4;
                nodeEpsilon[n] = C_mu * Math.Pow(nodeK[n], 1.5) / 0.1;
                nodeOmega[n] = nodeK[n] / (0.09 * 0.1);
                nodeEddyViscosity[n] = C_mu * nodeK[n] * nodeK[n] / nodeEpsilon[n];
                nodeEddyDiffusivity[n] = nodeEddyViscosity[n] / 0.7;
            }
        }
        ...
        private void UpdateVelocityField()
        {
            if (useUnstructuredGrid)
            {
                ...
                double[] newVelocityX = new double[nodes.Count];
                double[] newVelocityZ = new double[nodes.Count];
                for (int n = 0; n < nodes.Count; n++)
                {
                    double x = nodes[n].X, z = nodes[n].Z;
                    double localDepth = estuaryDepth * (0.5 + 0.5 * Math.Min(1.0, x / 200.0));
                    newVelocityX[n] = tidalAmplitude * Math.Sin(tidalPhase) * (1.0 - z / localDepth);
                    double drho_dx = ComputeNodeGradientX(n, nodeDensity);
                    double buoyancy = -g * drho_dx / rho0;
                    newVelocityZ[n] = nodeVelocityZ[n] + dt * buoyancy;
                    newVelocityZ[n] = Math.Max(-0.5, Math.Min(0.5, newVelocityZ[n]));
                }
                nodeVelocityX = newVelocityX;
                nodeVelocityZ = newVelocityZ;
            }
            else
            {
                double tidalPhase = 2.0 * Math.PI * time / tidalPeriod;
                for (int i = 0; i < gridSizeX; i++)
                {
                    ...
                    for (int j = 0; j < gridSizeZ; j++)
                    {
                        ...
                        velocityFieldZ[i, j] = Math.Max(-0.5, Math.Min(0.5, velocityFieldZ[i, j]));
                    }
                }
            }
        }

        private double ComputeNodeGradientX(int n, double[] field)
        {
            double sum = 0.0;
            double weightSum = 0.0;
            foreach (int t in nodes[n].NeighborTriangles)
            {
                Triangle tri = triangles[t];
                int n0 = tri.Nodes[0], n1 = tri.Nodes[1], n2 = tri.Nodes[2];
                double x0 = nodes[n0].X, x1 = nodes[n1].X, x2 = nodes[n2].X;
                double f0 = field[n0], f1 = field[n1], f2 = field[n2];
                double area = tri.Area;
                double gradX = ((f0 + f1 + f2) / 3.0) * (x0 + x1 + x2) / (3.0 * area);
                sum += gradX * area;
                weightSum += area;
            }
        ...
        public void ShowTVDWindow()
        {
            ...
            useUnstructuredGridCheckBox = new CheckBox
            {
                Location = new Point(580, 530),
                Size = new Size(150, 25),
                Text = "Use Unstructured Grid",
                Font = new Font("Verdana", 8F),
                Checked = false
            };
            useUnstructuredGridCheckBox.CheckedChanged += (s, e) =>
            {
                useUnstructuredGrid = useUnstructuredGridCheckBox.Checked;
                InitializeFields();
                visualizationPanel.Invalidate();
                outputTextBox.AppendText($"Switched to {(useUnstructuredGrid ? "unstructured" : "structured")} grid.\r\n");
            };
            ...
            tvdWindow.Controls.Add(useUnstructuredGridCheckBox);
        ...
        private void UpdateSimulation()
        {
            ...
            else
            {
                ...
                for (int i = 0; i < gridSizeX; i++)
                {
                    ...
                    for (int j = 0; j < gridSizeZ; j++)
                    {
                        int j_prev = (j == 0) ? 0 : j - 1;
                        int j_next = (j == gridSizeZ - 1) ? gridSizeZ - 1 : j + 1;
                        double x = i * dx;
                        ...
                        newDensity[i, j] = Math.Max(950.0, Math.Min(1050.0, newDensity[i, j]));
                        ...
                        newVelocityZ[i, j] = Math.Max(-0.5, Math.Min(0.5, newVelocityZ[i, j]));
                    }
                }
        ...
        private void UpdateUnstructuredSimulation()
        {
            double[] newSalinity = new double[nodes.Count];
            double[] newTemperature = new double[nodes.Count];
            double[] newDensity = new double[nodes.Count];
            double[] newK = new double[nodes.Count];
            double[] newEpsilon = new double[nodes.Count];
            double[] newOmega = new double[nodes.Count];
            double[] newVelocityZ = new double[nodes.Count];
            ...
            for (int n = 0; n < nodes.Count; n++)
            {
                double x = nodes[n].X, z = nodes[n].Z;
                double localDepth = estuaryDepth * (0.5 + 0.5 * Math.Min(1.0, x / 200.0));
                bool isBoundary = z <= 0 || z >= localDepth || x <= 0 || x >= estuaryLength;

                double salinitySource = x < 200.0 ? riverSalinitySource : (x > 800.0 ? oceanSalinitySource : 0.0);
                double tempSource = x < 200.0 ? riverTempSource : (x > 800.0 ? oceanTempSource : 0.0);

                double dudz = ComputeNodeGradientZ(n, nodeVelocityX);
                double drho_dz = ComputeNodeGradientZ(n, nodeDensity);
                double P_shear = nodeEddyViscosity[n] * dudz * dudz;
                double P_buoy = -g * nodeEddyDiffusivity[n] * drho_dz / rho0;
                double P = Math.Max(P_shear + P_buoy, 0);

                double fluxSalinity = 0.0, fluxTemperature = 0.0, fluxK = 0.0, fluxEpsilon = 0.0, fluxOmega = 0.0;
                double controlVolumeArea = 0.0;
                foreach (int t in nodes[n].NeighborTriangles)
                {
                    Triangle tri = triangles[t];
                    int n0 = tri.Nodes[0], n1 = tri.Nodes[1], n2 = tri.Nodes[2];
                    double avgVx = (nodeVelocityX[n0] + nodeVelocityX[n1] + nodeVelocityX[n2]) / 3.0;
                    double avgVz = (nodeVelocityZ[n0] + nodeVelocityZ[n1] + nodeVelocityZ[n2]) / 3.0;
                    double edgeLength = tri.Area * 2.0 / Math.Sqrt(tri.Area); // Approximate
                    double normalX = (nodes[n1].Z - nodes[n2].Z) / edgeLength;
                    double normalZ = -(nodes[n1].X - nodes[n2].X) / edgeLength;
                    double fluxVel = avgVx * normalX + avgVz * normalZ;
                    double fluxArea = isBoundary ? 0 : tri.Area / 3.0;
                    controlVolumeArea += fluxArea;

                    fluxSalinity += fluxArea * fluxVel * (fluxVel > 0 ? nodeSalinity[n] : (nodeSalinity[n0] + nodeSalinity[n1] + nodeSalinity[n2]) / 3.0);
                    fluxTemperature += fluxArea * fluxVel * (fluxVel > 0 ? nodeTemperature[n] : (nodeTemperature[n0] + nodeTemperature[n1] + nodeTemperature[n2]) / 3.0);
                    fluxK += fluxArea * fluxVel * (fluxVel > 0 ? nodeK[n] : (nodeK[n0] + nodeK[n1] + nodeK[n2]) / 3.0);
                    if (turbulenceModel == "k-epsilon")
                        fluxEpsilon += fluxArea * fluxVel * (fluxVel > 0 ? nodeEpsilon[n] : (nodeEpsilon[n0] + nodeEpsilon[n1] + nodeEpsilon[n2]) / 3.0);
                    else
                        fluxOmega += fluxArea * fluxVel * (fluxVel > 0 ? nodeOmega[n] : (nodeOmega[n0] + nodeOmega[n1] + nodeOmega[n2]) / 3.0);
                }

                newSalinity[n] = Math.Max(0.0, nodeSalinity[n] - dt * fluxSalinity / controlVolumeArea + dt * salinitySource);
                newTemperature[n] = Math.Max(15.0, nodeTemperature[n] - dt * fluxTemperature / controlVolumeArea + dt * tempSource);
                newDensity[n] = rho0 * (1.0 - alpha * (newTemperature[n] - T0) + beta_s * (newSalinity[n] - S0));
                newDensity[n] = Math.Max(950.0, Math.Min(1050.0, newDensity[n]));
                newK[n] = Math.Max(1e-6, nodeK[n] - dt * fluxK / controlVolumeArea + dt * (P - (turbulenceModel == "k-epsilon" ? nodeEpsilon[n] : beta_star * nodeK[n] * nodeOmega[n])));

                if (turbulenceModel == "k-epsilon")
                {
                    newEpsilon[n] = Math.Max(1e-6, nodeEpsilon[n] - dt * fluxEpsilon / controlVolumeArea + dt * (C1_epsilon * P * nodeEpsilon[n] / nodeK[n] - C2_epsilon * nodeEpsilon[n] * nodeEpsilon[n] / nodeK[n]));
                }
                else
                {
                    newOmega[n] = Math.Max(1e-6, nodeOmega[n] - dt * fluxOmega / controlVolumeArea + dt * (alpha_omega * P * nodeOmega[n] / nodeK[n] - beta * nodeOmega[n] * nodeOmega[n]));
                }

                newVelocityZ[n] = nodeVelocityZ[n]; // Updated in UpdateVelocityField
                nodeEddyViscosity[n] = turbulenceModel == "k-epsilon" ? C_mu * newK[n] * newK[n] / newEpsilon[n] : newK[n] / newOmega[n];
                nodeEddyDiffusivity[n] = nodeEddyViscosity[n] / 0.7;
            }

            nodeSalinity = newSalinity;
            nodeTemperature = newTemperature;
            nodeDensity = newDensity;
            nodeK = newK;
            if (turbulenceModel == "k-epsilon") nodeEpsilon = newEpsilon;
            else nodeOmega = newOmega;
            nodeVelocityZ = newVelocityZ;
        }

        private double ComputeNodeGradientZ(int n, double[] field)
        {
            double sum = 0.0;
            double weightSum = 0.0;
            foreach (int t in nodes[n].NeighborTriangles)
            {
                Triangle tri = triangles[t];
                int n0 = tri.Nodes[0], n1 = tri.Nodes[1], n2 = tri.Nodes[2];
                double z0 = nodes[n0].Z, z1 = nodes[n1].Z, z2 = nodes[n2].Z;
                double f0 = field[n0], f1 = field[n1], f2 = field[n2];
                double area = tri.Area;
                double gradZ = ((f0 + f1 + f2) / 3.0) * (z0 + z1 + z2) / (3.0 * area);
                sum += gradZ * area;
                weightSum += area;
            }
            return weightSum > 0 ? sum / weightSum : 0.0;
        ...
        private void VisualizationPanel_Paint(object sender, PaintEventArgs e)
        {
            ...
            if (useUnstructuredGrid)
            {
                ...
                {
                    int j = (int)(selectedDepth / (estuaryDepth / 25));
                    j = Math.Max(0, Math.Min(24, j));
                    List<double> valuesS = new List<double>();
                    List<double> valuesT = new List<double>();
                    List<double> valuesV = new List<double>();
                    for (int n = 0; n < nodes.Count; n++)
                    {
                        if (Math.Abs(nodes[n].Z - j * (estuaryDepth / 25)) < estuaryDepth / 50)
                        {
                            // Validate nodeSalinity, nodeTemperature, and velocity
                            if (!double.IsNaN(nodeSalinity[n]) && !double.IsInfinity(nodeSalinity[n]) &&
                                !double.IsNaN(nodeTemperature[n]) && !double.IsInfinity(nodeTemperature[n]) &&
                                !double.IsNaN(nodeVelocityX[n]) && !double.IsInfinity(nodeVelocityX[n]) &&
                                !double.IsNaN(nodeVelocityZ[n]) && !double.IsInfinity(nodeVelocityZ[n]))
                            {
                                valuesS.Add(nodeSalinity[n]);
                                valuesT.Add(nodeTemperature[n]);
                                double mag = Math.Sqrt(nodeVelocityX[n] * nodeVelocityX[n] + nodeVelocityZ[n] * nodeVelocityZ[n]);
                                valuesV.Add(mag);
                            }
                        }
                    }

                    // Only draw if we have enough data points
                    if (valuesS.Count > 1)
                    {
                        for (int i = 0; i < valuesS.Count - 1; i++)
                        {
                            float x1 = i * panelWidth / valuesS.Count;
                            float x2 = (i + 1) * panelWidth / valuesS.Count;
                            float y1 = panelHeight / 3 - (float)(Math.Min(Math.Max(valuesS[i], 0), 35.0) / 35.0 * panelHeight / 3);
                            float y2 = panelHeight / 3 - (float)(Math.Min(Math.Max(valuesS[i + 1], 0), 35.0) / 35.0 * panelHeight / 3);

                            // Validate coordinates
                            if (!float.IsNaN(x1) && !float.IsInfinity(x1) && !float.IsNaN(y1) && !float.IsInfinity(y1) &&
                                !float.IsNaN(x2) && !float.IsInfinity(x2) && !float.IsNaN(y2) && !float.IsInfinity(y2))
                            {
                    ...
                    if (valuesT.Count > 1)
                    {
                        for (int i = 0; i < valuesT.Count - 1; i++)
                        {
                            float x1 = i * panelWidth / valuesT.Count;
                            float x2 = (i + 1) * panelWidth / valuesT.Count;
                            // float y1 = 2 * panelHeight / 3 - (float)(Math.Min(Math.Max(valuesT[i], 15.0), 25.0) - 15.0) / 10.0 * panelHeight / 3;
                            float y1 = (float)(2 * panelHeight / 3 - (float)(Math.Min(Math.Max(valuesT[i], 15.0), 25.0) - 15.0) / 10.0 * panelHeight / 3);
                            // float y2 = 2 * panelHeight / 3 - (float)(Math.Min(Math.Max(valuesT[i + 1], 15.0), 25.0) - 15.0) / 10.0 * panelHeight / 3;
                            float y2 = (float)(2 * panelHeight / 3 - (float)(Math.Min(Math.Max(valuesT[i + 1], 15.0), 25.0) - 15.0) / 10.0 * panelHeight / 3);
                    ...
                    if (valuesV.Count > 1)
                    {
                        for (int i = 0; i < valuesV.Count - 1; i++)
                        {
                            float x1 = i * panelWidth / valuesV.Count;
                            float x2 = (i + 1) * panelWidth / valuesV.Count;
                            float y1 = panelHeight - (float)(Math.Min(Math.Max(valuesV[i], 0), tidalAmplitude) / tidalAmplitude * panelHeight / 3);
                            float y2 = panelHeight - (float)(Math.Min(Math.Max(valuesV[i + 1], 0), tidalAmplitude) / tidalAmplitude * panelHeight / 3);
                ...
                {
                    int i = (int)(selectedXPosition / (estuaryLength / 40));
                    ...
                    for (int n = 0; n < nodes.Count; n++)
                    {
                        if (Math.Abs(nodes[n].X - i * (estuaryLength / 40)) < estuaryLength / 80)
                        {
                            valuesS.Add(nodeSalinity[n]);
                            valuesT.Add(nodeTemperature[n]);
                            double mag = Math.Sqrt(nodeVelocityX[n] * nodeVelocityX[n] + nodeVelocityZ[n] * nodeVelocityZ[n]);
                    ...
                    for (int j = 0; j < valuesS.Count - 1; j++)
                    {
                        float z1 = j * panelHeight / valuesS.Count;
                        float z2 = (j + 1) * panelHeight / valuesS.Count;
                        float x1 = panelWidth / 3 * (float)(valuesS[j] / 35.0);
                        float x2 = panelWidth / 3 * (float)(valuesS[j + 1] / 35.0);
                        g.DrawLine(new Pen(Color.Blue, 2), x1, z1, x2, z2);
                    }
                    for (int j = 0; j < valuesT.Count - 1; j++)
                    {
                        float z1 = j * panelHeight / valuesT.Count;
                        float z2 = (j + 1) * panelHeight / valuesT.Count;
                        float x1 = panelWidth / 3 + panelWidth / 3 * (float)((valuesT[j] - 15.0) / 10.0);
                        float x2 = panelWidth / 3 + panelWidth / 3 * (float)((valuesT[j + 1] - 15.0) / 10.0);
                        g.DrawLine(new Pen(Color.Red, 2), x1, z1, x2, z2);
                    }
                    for (int j = 0; j < valuesV.Count - 1; j++)
                    {
                        float z1 = j * panelHeight / valuesV.Count;
                        float z2 = (j + 1) * panelHeight / valuesV.Count;
                        float x1 = 2 * panelWidth / 3 + panelWidth / 3 * (float)(valuesV[j] / tidalAmplitude);
                        float x2 = 2 * panelWidth / 3 + panelWidth / 3 * (float)(valuesV[j + 1] / tidalAmplitude);
                ...
                else if (visualizationMode == "ContourPlot")
                {
                    ...
                    {
                        float x0 = (float)(nodes[t.Nodes[0]].X * panelWidth / estuaryLength);
                        float y0 = (float)(nodes[t.Nodes[0]].Z * panelHeight / estuaryDepth);
                        float x1 = (float)(nodes[t.Nodes[1]].X * panelWidth / estuaryLength);
                        float y1 = (float)(nodes[t.Nodes[1]].Z * panelHeight / estuaryDepth);
                        float x2 = (float)(nodes[t.Nodes[2]].X * panelWidth / estuaryLength);
                        float y2 = (float)(nodes[t.Nodes[2]].Z * panelHeight / estuaryDepth);
                        PointF[] points = new[] { new PointF(x0, y0), new PointF(x1, y1), new PointF(x2, y2) };
                        double avgSalinity = (nodeSalinity[t.Nodes[0]] + nodeSalinity[t.Nodes[1]] + nodeSalinity[t.Nodes[2]]) / 3.0;
                        double avgTemperature = (nodeTemperature[t.Nodes[0]] + nodeTemperature[t.Nodes[1]] + nodeTemperature[t.Nodes[2]]) / 3.0;
                        int salinityColor = (int)(avgSalinity / 35.0 * 255);
                        salinityColor = Math.Max(0, Math.Min(255, salinityColor));
                        int tempColor = (int)((avgTemperature - 15.0) / 10.0 * 255);
                        ...
                        {
                            g.FillPolygon(brush, new[] { points[0], points[1], points[2] });
                        }
                    }
                }
                ...
                {
                    int step = 5;
                    float arrowScale = 1000.0f;
                    using (Pen arrowPen = new Pen(Color.Green, 1))
                    {
                        arrowPen.CustomEndCap = new AdjustableArrowCap(3, 3);
                        for (int n = 0; n < nodes.Count; n += step)
                        {
                            float x = (float)(nodes[n].X * panelWidth / estuaryLength);
                            float y = (float)(nodes[n].Z * panelHeight / estuaryDepth);
                            float u = (float)(nodeVelocityX[n] * arrowScale);
                            float v = (float)(-nodeVelocityZ[n] * arrowScale);
                    ...
                    using (Pen arrowPen = new Pen(Color.Green, 1))
                    {
                        ...
                        for (int i = 0; i < gridSizeX; i += step)
                        {
                            for (int j = 0; j < gridSizeZ; j += step)
                            {
                                ...
                                float v = (float)(-velocityFieldZ[i, j] * arrowScale);
                                g.DrawLine(arrowPen, x, y, x + u, y + v);
                            }
                        }
                    }
