...
namespace EstuarineCirculationModeling
{
    public class TidalStrainSim
    {
        ...
        private double riverDischargeAmplitude; // Peak river velocity (m/s)
        private double riverPeriod; // Seasonal period (s)
        ...
        private readonly double[] windSpeedHistory = new double[1000]; // Also stores river discharge
        ...
        private const double rhoA = 1.225; // Air density (kg/m³)
        private const double Cd = 0.0012; // Drag coefficient
        ...
        private const double kw = 0.1; // Wind mixing scaling factor
        ...
        private void UpdateSimulation()
        {
            ...
            // Compute time-varying wind stress
            ...
            // Compute time-varying river discharge
            double riverPhase = 2 * Math.PI * currentTime / riverPeriod;
            double baseDischarge = 0.1; // Base river velocity (m/s)
            double currentRiverDischarge = baseDischarge + riverDischargeAmplitude * Math.Sin(riverPhase);
            currentRiverDischarge = Math.Max(0.0, currentRiverDischarge); // Non-negative
            double riverSalinity = 10.0 * (1.0 - currentRiverDischarge / (baseDischarge + riverDischargeAmplitude)); // 0–10 PSU
            double riverTemperature = 14.0 - 4.0 * (currentRiverDischarge / (baseDischarge + riverDischargeAmplitude)); // 10–14°C
            double riverSediment = 200.0 * (currentRiverDischarge / (baseDischarge + riverDischargeAmplitude)); // 0–200 g/m³

            double dx = estuaryLength / (gridX - 1);
            double dz = estuaryDepth / (gridZ - 1);
            double dt = simulationTimer.Interval / 1000.0; // 0.01s

            // Update velocity profiles (Navier-Stokes: advection + viscosity + Coriolis + wind stress)
            ...
            for (int i = 1; i < gridX - 1; i++)
            {
                for (int j = 1; j < gridZ - 1; j++)
                {
                    ...
                    // Wind-driven Ekman transport (near surface)
                    double zFraction = (double)j / (gridZ - 1);
                    double windMixing = kw * tauW * (1.0 - zFraction); // Stronger near surface
                    ...
                    // Add turbulence with wind-enhanced mixing
                    double localTurbulence = turbulenceIntensity + windMixing;
                    newUVelocityProfile[i, j] += localTurbulence * tidalAmplitude * (rand.NextDouble() - 0.5);
                    newWVelocityProfile[i, j] += localTurbulence * tidalAmplitude * (rand.NextDouble() - 0.5);
                }
            }
            // Boundary conditions with wind stress and river inflow
            for (int i = 0; i < gridX; i++)
            {
                newUVelocityProfile[i, 0] = 0.0; // No-slip at bottom
                newUVelocityProfile[i, gridZ - 1] = tidalVelocity + tauWx / (rho0 * kinematicViscosity); // Wind stress at surface
                newWVelocityProfile[i, 0] = 0.0; // No-slip at bottom
                newWVelocityProfile[i, gridZ - 1] = tauWz / (rho0 * kinematicViscosity); // Wind stress at surface
            }
            for (int j = 0; j < gridZ; j++)
            {
                newUVelocityProfile[0, j] = currentRiverDischarge; // Dynamic river inflow
            ...
            for (int j = 0; j < gridZ; j++)
            {
                newSalinityProfile[0, j] = riverSalinity; // Dynamic river inflow
            ...
            for (int j = 0; j < gridZ; j++)
            {
                newTemperatureProfile[0, j] = riverTemperature; // Dynamic river inflow
                newTemperatureProfile[gridX - 1, j] = 15.0; // Ocean
            }
            for (int i = 0; i < gridX; i++)
            {
            ...
            for (int j = 0; j < gridZ; j++)
            {
                newSedimentProfile[0, j] = riverSediment; // Dynamic river inflow
            ...
            windSpeedHistory[historyIndex] = currentRiverDischarge; // Store river discharge
        ...
        private void VisualizationBox_Paint(object sender, PaintEventArgs e)
        {
            ...
            g.DrawString($"Wind: {currentWindSpeed:F2} m/s, {currentWindDirection:F0}°", new Font("Consolas", 10), Brushes.Black, 10 / zoomLevel, 30 / zoomLevel);
            ...
            g.DrawString($"River: {currentRiverDischarge:F2} m/s", new Font("Consolas", 10), Brushes.Black, 10 / zoomLevel, 50 / zoomLevel);
        }
        ...
        private void TimeSeriesBox_Paint(object sender, PaintEventArgs e)
        {
            ...
            g.DrawString("Ri / Rρ / C / Q_r", new Font("Consolas", 8), Brushes.Black, 30, 10);
            ...
            g.DrawString("Q_r", new Font("Consolas", 8), Brushes.Black, 90, 60);
        }
