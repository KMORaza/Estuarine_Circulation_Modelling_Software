...
namespace EstuarineCirculationModeling
{
    public class CompForcingMechanism : Form
    {
        ...
        private Label riverDischargeLabel, tidalAmplitudeLabel, tidalPeriodLabel, windSpeedLabel, windDirectionLabel, salinityGradientLabel, waveHeightLabel, stormSurgeAmplitudeLabel, solverTypeLabel;
        private TextBox riverDischargeTextBox, tidalAmplitudeTextBox, tidalPeriodTextBox, windSpeedTextBox, windDirectionTextBox, salinityGradientTextBox, waveHeightTextBox, stormSurgeAmplitudeTextBox;
        ...
        private void InitializeComponents()
        {
            ...
            controlPanel.Controls.AddRange(new Control[]
            {
                ...
                solverTypeLabel, solverTypeComboBox, startButton, pauseButton, resetButton
            });
        ...
        private void InitializeSimulation()
        {
            ...
            else if (solverType == "2D Shallow Water")
            {
                swe2DSolver = new ShallowWaterEq2D(estuaryLength, estuaryWidth, estuaryDepth, nx, ny,
                    riverDischarge, tidalAmplitude, tidalPeriod, windSpeed, windDirection,
                    salinityGradient, waveHeight, stormSurgeAmplitude);
            }
            else // 1D Simplified
            {
        ...
        private void UpdateSimulation()
        {
                ...
                for (int i = 1; i < nx - 1; i++)
                    for (int j = 1; j < ny - 1; j++)
                        for (int k = 1; k < nz - 1; k++)
                        {
                            ...
                            // Apply flux limiters to advection terms
            ...
            {
                swe2DSolver.Update(currentTime);
            ...
            else
            {
                ...
                for (int i = 1; i < gridPoints - 1; i++)
                {
                    ...
                    double coriolis = coriolisParameter * velocityProfile[i]; // Simplified for 1D (acting on along-channel velocity)
        ...
        private void startButton_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                UpdateButtonStates();
                if (!outputConsoleTextBox.IsDisposed)
                    outputConsoleTextBox.AppendText($"Simulation started ({solverType} solver).\r\n");
            }
            catch (Exception ex)
            {
        ...
        private void visualizationPanel_Paint(object sender, PaintEventArgs e)
        {
            ...
            else if (solverType == "2D Shallow Water")
            {
                ...
                using (Pen pen = new Pen(Color.Green, 2))
                {
                    float maxEta = (float)etaAvg.Max(Math.Abs);
                    if (maxEta == 0 || float.IsNaN(maxEta) || float.IsInfinity(maxEta)) maxEta = tidalAmplitude > 0 ? (float)tidalAmplitude : 1.0f;
                    for (int i = 0; i < nx - 1; i++)
                    {
                        float x1 = i * panelWidth / (nx - 1);
                        float x2 = (i + 1) * panelWidth / (nx - 1);
                        float y1 = panelHeight / 2 - (float)(etaAvg[i] / maxEta) * panelHeight / 8;
                        float y2 = panelHeight / 2 - (float)(etaAvg[i + 1] / maxEta) * panelHeight / 8;
                        if (float.IsNaN(y1) || float.IsInfinity(y1) || float.IsNaN(y2) || float.IsInfinity(y2)) continue;
                        y1 = Math.Max(0, Math.Min(panelHeight, y1));
                        y2 = Math.Max(0, Math.Min(panelHeight, y2));
                        g.DrawLine(pen, x1, y1, x2, y2);
