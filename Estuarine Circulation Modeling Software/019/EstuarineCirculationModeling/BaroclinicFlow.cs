...
namespace EstuarineCirculationModeling
{
    public class BaroclinicFlow
    {
        ...
        // Compute density using UNESCO 1983 equation of state
        public double CalculateDensity(double salinity, double temperature)
        {
            // Estimate pressure (dbar) at estuary depth: P = ρ₀ * g * h / 10^4
            double pressure = densityReference * g * estuaryDepth / 1e4;
            return EqOfState.ComputeDensity(salinity, temperature, pressure);
        }
        ...
        private double ComputeSurfaceHeatFlux(double waterTemp, double velocityGradient, double currentTime)
        {
            ...
            // Sensible heat: Q_sensible = ρ_air * C_p_air * C_h * U * (T_w - T_a) * bowenRatio
            ...
            double sensibleFlux = airDensity * airSpecificHeat * sensibleHeatCoefficient * windSpeed * (waterTemp - atmosphericTemperature) * bowenRatio;
