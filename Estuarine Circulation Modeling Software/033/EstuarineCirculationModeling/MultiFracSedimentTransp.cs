...
namespace EstuarineCirculationModeling
{
    public class MultiFracSedimentTransp : Form
    {
        ...
        private void UpdateSimulation()
        {
            ...
            time += timeStep;
            visualizationPanel.Invalidate();
            if (showHovmoellerCheckBox.Checked)
                hovmoellerPanel.Invalidate();
            budgetMapPanel.Invalidate();
            correlationPanel.Invalidate();
            UpdateOutput();
        ...
        private void HovmoellerPanel_Paint(object sender, PaintEventArgs e)
        {
            ...
            if (sedimentFluxes.Count == 0)
            {
                g.DrawString("No flux data available", new Font("Verdana", 11), Brushes.Black, 10, 10);
                return;
            }
            ...
            // Smooth fluxes using a simple moving average
            double[] smoothedFluxes = new double[sedimentFluxes.Count * gridPoints];
            for (int t = 0; t < sedimentFluxes.Count; t++)
            {
                for (int i = 1; i < gridPoints - 1; i++)
                {
                    double avg = (sedimentFluxes[t][selectedSediment * gridPoints + i - 1] +
                                  sedimentFluxes[t][selectedSediment * gridPoints + i] +
                                  sedimentFluxes[t][selectedSediment * gridPoints + i + 1]) / 3.0;
                    smoothedFluxes[t * gridPoints + i] = avg;
                }
                smoothedFluxes[t * gridPoints] = sedimentFluxes[t][selectedSediment * gridPoints];
                smoothedFluxes[t * gridPoints + gridPoints - 1] = sedimentFluxes[t][selectedSediment * gridPoints + gridPoints - 1];
            }

            int maxDisplaySteps = Math.Min(sedimentFluxes.Count, maxTimeSteps);
            for (int t = 0; t < maxDisplaySteps; t++)
            {
                float x = t * panelWidth / (float)maxDisplaySteps;
                float width = panelWidth / (float)maxDisplaySteps;
                for (int i = 0; i < gridPoints; i++)
                {
                    float y = i * panelHeight / (float)gridPoints;
                    float height = panelHeight / (float)gridPoints;
                    double flux = smoothedFluxes[t * gridPoints + i];
                    double logFlux = flux != 0 ? Math.Sign(flux) * Math.Log10(Math.Abs(flux) / 1e-6 + 1) : 0;
                    double logFluxRange = Math.Log10(fluxRange / 1e-6 + 1);
                    int intensity = logFluxRange > 0 ? (int)(255 * logFlux / logFluxRange) : 0;
            ...
            float yStart = 0, yEnd = 0; // Initialize to avoid undefined variables
            for (int i = 0; i < gridPoints; i++)
            {
                if (salinityField[i] >= 5.0 && salinityField[i] <= 15.0)
                {
            ...
            if (etmStart != -1)
            {
                yStart = etmStart * panelHeight / (float)gridPoints;
                yEnd = (etmEnd + 1) * panelHeight / (float)gridPoints;
                g.FillRectangle(new SolidBrush(Color.FromArgb(50, 0, 255, 0)), 0, yStart, panelWidth, yEnd - yStart);
            }

            // Draw axes and labels
            g.DrawLine(Pens.Black, 0, panelHeight - 10, panelWidth, panelHeight - 10);
            g.DrawLine(Pens.Black, 10, 0, 10, panelHeight);
            g.DrawString($"Time (s) [0, {(maxDisplaySteps * timeStep):F2}]", new Font("Verdana", 11), Brushes.Black, panelWidth - 120, panelHeight - 30);
            g.DrawString("Distance (m)", new Font("Verdana", 11), Brushes.Black, 10, 10);
            g.DrawString($"{sedimentTypes[selectedSediment]} Flux (kg/m²s)", new Font("Verdana", 11), Brushes.Black, 10, 30);
            if (etmStart != -1) // Only use yStart and yEnd if defined
            {
                g.DrawString("ETM Zone", new Font("Verdana", 10), Brushes.Black, 10, (yStart + yEnd) / 2 - 10);
            }
