...
namespace EstuarineCirculationModeling
{
    public class AsymmTidalMix
    {
        ...
        private readonly SimpsonHunterMechanismParam tidalEffectsModel; // New tidal effects model
        ...
        private void UpdateSimulation()
        {
            ...
            foreach (Cell cell in cells)
            {
                ...
                for (int k = 0; k < numSigmaLayers; k++)
                {
                    ...
                    if (i > 0 && cell.U[k] > 0)
                    {
                        uAdv = (cell.U[k] + stokesDrift) * (cell.U[k] - cells[i - 1].U[k]) / avgCellWidth;
                        vAdv = cell.U[k] * (cell.V[k] - cells[i - 1].V[k]) / avgCellWidth;
                    }
                    else if (i < numCells - 1 && cell.U[k] < 0)
                    {
                        uAdv = (cell.U[k] + stokesDrift) * (cells[i + 1].U[k] - cell.U[k]) / avgCellWidth;
            ...
            foreach (Cell cell in cells)
            {
                ...
                for (int k = 0; k < numSigmaLayers; k++)
                {
                    ...
                    kProd += mixingModel.ComputeShearStrainProduction(cell, k, depth, sigmaStep, nuT); // Add shear-strain production
                    ...
                    if (i > 0 && cell.U[k] > 0)
                        ...
                        turbidityChange -= cell.U[k] * (cells[i + 1].Turbidity[k] - cell.Salinity[k]) / avgCellWidth;
                    if (k < numSigmaLayers - 1)
                    {
                        double mixingCoeff = kinematicViscosity + nuT / 1.0;
        ...
        private void UpdateOutputConsole()
        {
            ...
            // Compute average Richardson number
